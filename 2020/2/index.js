...
function part1Answer() {
  ...
  for (let i = 0; i < passwordSpreadsheet.length; i++) {
    let split = splitPasswordAndRule(passwordSpreadsheet[i]);
    let password = split[0];
    let ruleLetter = split[1];
    let ruleMin = split[2];
    let ruleMax = split[3];
    let validPassword = isValidPasswordPt1(
      password,
      ruleLetter,
      ruleMin,
      ruleMax
...
function part2Answer() {
  ...
  for (let i = 0; i < passwordSpreadsheet.length; i++) {
    ...
    let password = split[0];
    let ruleLetter = split[1];
    let position1 = split[2];
    let position2 = split[3];
    let validPassword = isValidPasswordPt2(
      password,
      ruleLetter,
      position1,
      position2
...
function isValidPasswordPt1(password, ruleLetter, ruleMin, ruleMax) {
  let valid = false;
  let ruleLetterRegEx = new RegExp(ruleLetter, "g");
  if (!password.match(ruleLetterRegEx)) {
    return valid;
  }

  let count = password.match(ruleLetterRegEx).length;
...
function isValidPasswordPt2(password, ruleLetter, position1, position2) {
    let valid = false;
    if (
      password[position1 - 1] === ruleLetter &&
      password[position2 - 1] === ruleLetter
    ) {
      valid = false;
    } else if (
      password[position1 - 1] !== ruleLetter &&
      ...
      valid = false;
    } else if (password[position1 - 1] === ruleLetter) {
      valid = true;
    } else if (password[position2 - 1] === ruleLetter) {
      valid = true;
    }
    return valid;
  }

function splitPasswordAndRule(password) {
  let splitPassword = password.split(":");
  let userPassword = splitPassword[1].trim();
  let splitRuleAndPassword = splitPassword[0].split(" ");
  let letter = splitRuleAndPassword[1];
  let ruleMin = splitRuleAndPassword[0].split("-")[0];
  let ruleMax = splitRuleAndPassword[0].split("-")[1];
  return [userPassword, letter, ruleMin, ruleMax];
}
