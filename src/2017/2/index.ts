...
export function checksum(rows: number[][], callback: Function): number {
  let total: number = 0;
  rows.forEach((row) => {
    total += callback(row);
  });
...
const answer1 = checksum(input, rowDifference);

export function evenlyDivisible(list:number[]): number {
...
const answer2 = checksum(input, evenlyDivisible)
