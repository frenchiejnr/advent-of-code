...
function repeatedFrequency(input) {
    ...
    for (let i = 0; i < functionInput.length; i++) {
        total += functionInput[i];
        if (totalArray.includes(total)) {
            return total;
        }
        totalArray.push(total);
        if (totalArray.length === functionInput.length + 1) {
            functionInput.push(...input);
        }
    }
    return total;
}
...
module.exports = {calculateFrequency, repeatedFrequency};
